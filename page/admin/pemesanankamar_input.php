	...
	<form method="post" class="d-flex gap-5 mt-5">
		...
		<div class="row">
			...
			<div class="mb-3">
				...
				<input type="text" name="id_tipe_kamar" class="form-control" id="id_tipe_kamar" hidden>
			</div>
			<div class="mb-3">
		 		<label>Harga</label>
		    ...
		    while ($data3 = mysqli_fetch_array($hasil3)) { ?>
		        <div class="form-group">
		            <div class="form-check">
  						<input type="checkbox" name="tmbhn[]" id="tambahan" class="form-check-input" value="<?= $data3['nama']."|".$data3['harga']; ?>" onclick="showJumlah(this)">
  		                <label class="form-check-label"><?= $data3['nama']; ?> (<?= $data3['harga']; ?>)</label>
		            </div>
		            <div class="form-group">
               		<label for="jumlah_<?= $data3['nama'] ?>">Jumlah:</label>
                	<input type="number" name="tmbhn_<?= $data3['nama'] ?>_jml" id="jumlah_<?= $data3['nama'] ?>" min="0" max="10" step="1" value="0" style="display:none">
            </div>
		        </div>
   		 	<?php } ?>
			...
			<div class="mb-3">
				<label>Diskon</label>
				<input type="checkbox" id="chkDiskon" name="chkDiskon"> Aktifkan diskon
				<select id="diskon" name="diskon" class="form-select" disabled>
			 		<?php 
			 			$sql1 = "SELECT * FROM tblDiskon";
			 			$hasil1 = mysqli_query($config,$sql1);
			 			while ($data1 = mysqli_fetch_array($hasil1)) { ?>
				 		<option value="<?= $data1['diskon']; ?>" ><?= $data1['diskon']; ?></option>
				 		<?php 
				 		} ?>
			 	</select>
			</div>
...
<?php 
	...
	if (isset($_POST['pilih'])) {
		...
		$harga_diskon = $sub_total*($diskon/100);
		$setelahdiskon = $sub_total - $harga_diskon;
		$hPajak = $setelahdiskon * $pajak;
		$totalharga = $setelahdiskon + $hPajak;
		...
		if($stok != 0){   
            $insert = mysqli_query($config, "INSERT INTO pemesanan VALUES ('$noktp','$kodepemesanan', '$kamar',  '$checkin', '$checkout', '$totalinap', '$jenishari', '$hargakamar', '$selected_items_str', '$totaltmbhn','$sub_total', '$harga_diskon ', '$hPajak', '$totalharga', '$waktuorder','$proses')");
            $stok = $stok-1;
...
function showJumlah(el) {
    var jumlahEl = document.getElementById("jumlah_"+el.value.split("|")[0]);
    if (el.checked) {
        jumlahEl.style.display = "block";
    } else {
        jumlahEl.style.display = "none";
    }
}

const chkDiskon = document.getElementById("chkDiskon");
const diskon = document.getElementById("diskon");

chkDiskon.addEventListener("change", function() {
	if (this.checked) {
